...
function New-SelfLockProcess([int]$processId) {
  return [pscustomobject]@{ name = 'TjuaeUI installer'; pid = $processId }
}
...
try {
  ...
  if ($targetPathFull -and (Test-Path -LiteralPath $targetPathFull -PathType Leaf)) {
    ...
    $knownRelative = @(
      'TjuaeUI.exe',
      'Uninstall TjuaeUI.exe',
      'resources\app.asar',
      'resources\app-update.yml',
      'resources\bundled-tjuaecore\win32-x64\tjuaecore.exe'
    )
    $known = @(
      $knownRelative |
...
namespace TjuaeUI.RestartManager {
...
'@
  ...
  $result = [TjuaeUI.RestartManager.Native]::RmStartSession([ref]$sessionHandle, 0, $key)
  if ($result -ne 0) {
  ...
  try {
    for ($i = 0; $i -lt $resources.Count; $i += 256) {
      ...
      $result = [TjuaeUI.RestartManager.Native]::RmRegisterResources($sessionHandle, [uint32]$chunk.Count, $chunk, 0, [IntPtr]::Zero, 0, $null)
      if ($result -ne 0) {
    ...
    for ($attempt = 0; $attempt -lt 6; $attempt++) {
      ...
      $count = [uint32]0
      $reasons = [uint32]0
      $result = [TjuaeUI.RestartManager.Native]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $null, [ref]$reasons)
    ...
    if ($result -eq $ERROR_MORE_DATA -or $needed -gt 0) {
      for ($attempt = 0; $attempt -lt 6; $attempt++) {
        ...
        $result = [TjuaeUI.RestartManager.Native]::RmGetList($sessionHandle, [ref]$needed, [ref]$count, $apps, [ref]$reasons)
    ...
    [void][TjuaeUI.RestartManager.Native]::RmEndSession($sessionHandle)
  }
} catch {
