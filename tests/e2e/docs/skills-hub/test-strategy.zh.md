...
- ✅ SQLite 数据库独立(`tjuaeui.db`)
- ✅ 扩展路径指向示例目录(`TJUAE_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.tjuaeui/skills/',     // My Skills 存储目录
...
- `/usr/local/share/tjuae-skills` — 系统级共享路径
...
- E2E 框架已设置 `TJUAE_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tjuaeui-e2e-custom-'));
...
| `auto-skill-card-${normalizeTestId(skill.name)}`      | 自动注入技能卡片            | line ~520 `<div className="skill-card">`        | `auto-skill-card-tjuaeui-skills`      |
