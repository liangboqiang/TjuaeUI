...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tjuaeui-e2e-state-'));
...
```typescript
TJUAE_EXTENSION_STATES_FILE: process.env.TJUAE_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `tjuaeui.db` → parallel execution impossible without refactoring.
...
1. Extend `TJUAEUI_E2E_TEST` to include worker ID:
   ```typescript
   TJUAEUI_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.TJUAEUI_E2E_TEST_WORKER_ID || '0';
   return `TjuaeUI-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/TjuaeUI-E2E-Worker-0/config/tjuaeui.db`
   - `~/Library/Application Support/TjuaeUI-E2E-Worker-1/config/tjuaeui.db`
...
**Complexity**: High (requires main process to read `TJUAEUI_DATABASE_PATH`, conflicts with userData convention)
