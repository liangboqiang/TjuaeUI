# Tjuae CLI (tjuaecli) E2E 测试 - 讨论记录

**项目**: TjuaeUI E2E Coverage
**子组**: chat-tjuaecli
**日期**: 2026-04-22
...
**参与人**: chat-tjuaecli-analyst

**完成内容**:

1. 分析 tjuaecli 源码（guid 入口、对话页、进程端、权限枚举、DB schema）
2. 起草 `requirements.zh.md` 初稿
3. 识别 8 个重大不确定性（模型切换、权限存储范围、binary 来源等）

**待办事项**:

- [x] chat-tjuaecli-designer 审核需求，补充测试用例设计建议
- [ ] chat-tjuaecli-engineer 审核需求，评估技术可行性
...
4. **清理顺序**（避免外键冲突）:
   ...
   await db.exec("DELETE FROM conversations WHERE name LIKE 'E2E-tjuaecli-%'");

   // 3. 清理 FS
   await fs.rm('/tmp/e2e-tjuaecli-*', { recursive: true });
...
| 4. CI 环境 binary 来源？            | 跳过 CI E2E（C 选项） + 本地测试套件 | `test.beforeAll()` 检查 binary，不存在则 `test.skip('tjuae-cli binary not found')`     |
...
1. **guid 页 data-testid 已有**（之前误判为 0 个）
   - 现有属性: `data-agent-pill="true"`, `data-agent-backend="tjuaecli"`, `data-agent-selected`
   - 位置: `AgentPillBar.tsx:79-82`

2. **对话页需新增 15+ testid**（否则无法定位元素）
   - **P0 优先级**（阻塞测试）: 5 个
     ```tsx
     [data-testid="tjuaecli-sendbox"]
     [data-testid="tjuaecli-model-selector"]
     [data-testid="agent-mode-selector-tjuaecli"]
...
3. **DB 清理需补充 helper**
   - 文件位置: `tests/e2e/helpers/chatTjuaeCli.ts`
   - 函数: `cleanupE2ETjuaeCliConversations()`, `getTjuaeCliMessages()` 等

4. **binary 已验证可用**
   - 版本: tjuaecli v0.1.12
   - 路径: `/Users/developer/.local/bin/tjuae-cli`
...
| 4. CI 环境 binary 来源？            | 短期 **CI skip**，长期 DevOps 配置       | `test.skip(() => resolveTjuaeCliBinary() === null, 'tjuae-cli binary not found')`         |
...
**背景**: 当前存储在 `TjuaeCliApprovalStore`（内存），进程重启后失效
...
**背景**: E2E 需依赖 tjuae-cli binary

**选项**:

- A) CI 预装（需 DevOps 配置）
- B) 测试前动态下载（需提供 URL + 版本锁定）
- C) 跳过 tjuaecli E2E（标记 skip + 原因）

**engineer 建议**: 先实现 **local E2E**，CI 策略延后

- **短期**: CI 跳过 tjuaecli E2E（`test.skip(() => resolveTjuaeCliBinary() === null, 'tjuae-cli binary not found')`）
- **长期**: DevOps 配置预装 或 动态下载（需 team-lead 协调）

**team-lead 决策**: **选 C CI 跳过**（短期）

- **理由**: 长期 CI 策略延后，不阻塞本轮 E2E 交付
- **实施**: `test.skip(() => resolveTjuaeCliBinary() === null, 'tjuae-cli binary not found')`
...
1. **权限命名统一**：使用 tjuaecli runtime capabilities 上报的 `auto_edit`，避免与 Gemini 的 `autoEdit` 混淆
...
- [ ] chat-tjuaecli-engineer review 用例设计，评估实现工作量
...
```typescript
const PROVIDER_BASED_AGENTS = new Set(['gemini', 'tjuaecli']);
const isGeminiMode =
...
**结论**: **备注不成立** — `isGeminiMode` 对 `tjuaecli` 默认为 **true**（`PROVIDER_BASED_AGENTS` 包含 tjuaecli）

**建议调整**:

- 删除 TC-A-04 备注："若 guid 页模型选择器未启用...改为 TC-A-07"
- 保留 TC-A-04 操作步骤 2："打开模型选择器（`GuidModelSelector`），选择第二个模型"
- 补充 TC-A-04 前置条件："验证 `isGeminiMode=true`（tjuaecli 默认启用）"
...
- `TjuaeCliManager.ts:766-772` 调用 `agent.denyTool(callId, reason)` 发送 `tool_deny` 命令
...
**问题**（L757）:
操作步骤称"设置环境变量：`process.env.TJUAE_CLI_PATH = '/dev/null'`"

**E2E 环境评估**:

- `process.env` 在 **Node.js 测试进程**中修改，但 **不影响已启动的 Electron 子进程**
- tjuae-cli binary 解析在 **main process**（`binaryResolver.ts`），读取 `process.env.TJUAE_CLI_PATH`
...
- 方案 A（推荐）：在 `test.beforeAll()` **启动 Electron 前** 设置环境变量
  ```typescript
  test.beforeAll(async () => {
    process.env.TJUAE_CLI_PATH = '/dev/null';
    // Electron 启动会继承此环境变量
  });
  ```
- 方案 B（更复杂）：通过 `electronApp.evaluate()` 动态修改 main process 环境
  ...
  ```
- **TC-A-13 调整**：操作步骤改为"在测试启动前设置 `TJUAE_CLI_PATH=/dev/null`（通过 `test.beforeAll` 或命令行 `TJUAE_CLI_PATH=/dev/null npm run test:e2e`）"
...
| 审核点          | 状态          | 调整建议                                                |
| --------------- | ------------- | ------------------------------------------------------- |
| 1. TC-A-04 备注 | ⚠️ 不成立     | 删除"改为 TC-A-07"备注，tjuaecli 默认启用模型选择器     |
