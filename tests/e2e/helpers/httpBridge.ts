 ...
 * HTTP calls against `tjuaecore` via `fetch('http://127.0.0.1:<port>/api/...')`.
...
export async function httpInvoke<T = unknown>(
  ...
  return page.evaluate(
    async ({ method: m, path: p, body: b }) => {
      ...
      // body even when the operation takes no body. Send `{}` as the default
      // body for DELETE so the request follows the same bridge contract.
