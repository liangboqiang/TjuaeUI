/**
 * TjuaeCli Chat E2E Tests - Mid-Conversation Switch (P1)
 *
 * Test Cases Covered:
...
test.describe('TjuaeCli Chat - Mid-Conversation Switch (P1)', () => {
  ...
  test.skip('TC-A-08: should handle continuous switch (model → permission → model)', async ({ page }) => {
    // SKIP: Pending tjuae-cli binary investigation - runtime provider+mode switch causes silent hang on subsequent messages
    ...
    const conversationName = `E2E-tjuaecli-${timestamp}-continuous-switch`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-tjuaecli/tc-a-08/01-created.png`);

      // Step 2: Send initial message
      await sendTjuaeCliMessage(page, conversationId, 'Hello, initial message.');
      await waitForTjuaeCliReply(page, conversationId);

      // Screenshot 02: initial reply
      await takeScreenshot(page, `chat-tjuaecli/tc-a-08/02-initial-reply.png`);
      ...
      await takeScreenshot(page, `chat-tjuaecli/tc-a-08/03-model-switched.png`);

      // Step 5: Switch permission (mode switch)
      const modeSelector = page.locator('[data-testid="agent-mode-selector-tjuaecli"]');
      await expect(modeSelector).toBeVisible({ timeout: 10000 });
      await modeSelector.click();
      await page.waitForTimeout(500);

      const yoloOption = page.locator('[data-testid="tjuaecli-mode-option-yolo"]');
      await expect(yoloOption).toBeVisible();
      await yoloOption.click();
      await page.waitForTimeout(1000);

      // Screenshot 04: permission switched
      await takeScreenshot(page, `chat-tjuaecli/tc-a-08/04-permission-switched.png`);

      // Step 6: Switch back to modelA (model switch #2)
      await modelSelector.click();
      await page.waitForTimeout(500);

      const firstModel = page.locator(`[data-testid="tjuaecli-model-option-${preconditions.models!.modelA.useModel}"]`);
      await firstModel.waitFor({ state: 'visible', timeout: 5000 });
      await firstModel.click();
      await page.waitForTimeout(1000);

      // Screenshot 05: model switched again
      await takeScreenshot(page, `chat-tjuaecli/tc-a-08/05-model-switched-again.png`);

      // Step 7: Send message after all switches
      await sendTjuaeCliMessage(page, conversationId, 'After all switches.');
      await waitForTjuaeCliReply(page, conversationId);

      // Screenshot 06: final reply
      await takeScreenshot(page, `chat-tjuaecli/tc-a-08/06-final-reply.png`);
      ...
      // 3. Verify all AI replies exist (message.status check not applicable for tjuaecli)
  ...
  test.skip('TC-A-09: should handle 3 rounds of conversation after model/permission switch', async ({ page }) => {
    // SKIP: Same root cause as TC-A-08 - tjuae-cli binary runtime state handling issue after model+permission switches
    // See tests/e2e/docs/chat-tjuaecli/implementation-mapping.zh.md "Known Issues" section
    ...
    const timestamp = Date.now();
    const conversationName = `E2E-tjuaecli-${timestamp}-multi-round`;
    ...
    try {
      ...
      // Step 2: Send initial message
      await sendTjuaeCliMessage(page, conversationId, 'Round 1: Initial message.');
      await waitForTjuaeCliReply(page, conversationId);

      // Screenshot 01: after round 1
      await takeScreenshot(page, `chat-tjuaecli/tc-a-09/01-round1.png`);
      ...
      await takeScreenshot(page, `chat-tjuaecli/tc-a-09/02-after-switches.png`);

      // Step 6: Round 2 - send and wait
      await sendTjuaeCliMessage(page, conversationId, 'Round 2: After model and permission switch.');
      await waitForTjuaeCliReply(page, conversationId);

      // Screenshot 03: after round 2
      await takeScreenshot(page, `chat-tjuaecli/tc-a-09/03-round2.png`);

      // Step 7: Round 3 - send and wait
      await sendTjuaeCliMessage(page, conversationId, 'Round 3: Continue with switched config.');
      await waitForTjuaeCliReply(page, conversationId);

      // Screenshot 04: after round 3
      await takeScreenshot(page, `chat-tjuaecli/tc-a-09/04-round3.png`);

      // Step 8: Round 4 (bonus) - send and wait
      await sendTjuaeCliMessage(page, conversationId, 'Round 4: Final message.');
      await waitForTjuaeCliReply(page, conversationId);

      // Screenshot 05: after round 4
      await takeScreenshot(page, `chat-tjuaecli/tc-a-09/05-round4.png`);
