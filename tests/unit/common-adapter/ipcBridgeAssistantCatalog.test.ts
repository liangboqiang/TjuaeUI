...
describe('ipcBridge assistant catalog adapter', () => {
  beforeEach(() => {
    httpBridgeMocks.calls.length = 0;
  });

  it('keeps categories when saving assistant settings', async () => {
    const { assistants } = await import('@/common/adapter/ipcBridge');

    await assistants.updateCatalogSettings.invoke({
      source: 'mine',
      namespace: '~',
      slug: 'reporter',
      name: '报告助手',
      ...
      categories: ['办公', '写作'],
      defaults: {
        model: { mode: 'remember_last' },
        permission: { mode: 'fixed', value: 'read-only' },
        thoughtLevel: { mode: 'fixed', value: 'medium' },
        skills: [],
        mcps: [],
      },
      recommendedPrompts: ['整理本周报告'],
      rules: '使用简洁格式。',
    });

    expect(httpBridgeMocks.calls).toContainEqual({
      method: 'PUT',
      path: '/api/assistants/catalog/mine/~/reporter/settings',
      body: {
        name: '报告助手',
        description: '整理报告',
        avatar: undefined,
