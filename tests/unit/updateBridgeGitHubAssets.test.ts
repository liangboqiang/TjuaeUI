...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/liangboqiang/TjuaeUI/releases/tag/v1.9.22',
    published_at: '2026-04-29T00:00:00Z',
    prerelease: false,
    draft: false,
    assets: [
      {
        name: 'TjuaeUI-1.9.22-mac-arm64.dmg',
        browser_download_url:
          'https://github.com/liangboqiang/TjuaeUI/releases/download/v1.9.22/TjuaeUI-1.9.22-mac-arm64.dmg',
        size: 123,
        content_type: 'application/x-apple-diskimage',
      },
      {
        name: 'TjuaeUI-1.9.22-win-x64.exe',
        browser_download_url:
          'https://github.com/liangboqiang/TjuaeUI/releases/download/v1.9.22/TjuaeUI-1.9.22-win-x64.exe',
        size: 456,
        content_type: 'application/vnd.microsoft.portable-executable',
      },
      {
        name: 'TjuaeUI-1.9.22-linux-x64.deb',
        browser_download_url:
          'https://github.com/liangboqiang/TjuaeUI/releases/download/v1.9.22/TjuaeUI-1.9.22-linux-x64.deb',
        size: 789,
      },
    ],
  },
...
describe('updateBridge GitHub release assets', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('uses the GitHub browser download URL without an external mirror', async () => {
    ...
    try {
      ...
      const result = await handler({ repo: 'liangboqiang/TjuaeUI' });
      ...
      const macAsset = assets.find((a: { name: string }) => a.name === 'TjuaeUI-1.9.22-mac-arm64.dmg');
      expect(macAsset).toBeDefined();
      expect(macAsset?.url).toBe(
        'https://github.com/liangboqiang/TjuaeUI/releases/download/v1.9.22/TjuaeUI-1.9.22-mac-arm64.dmg'
      );
      expect(macAsset?.url).toContain('github.com/liangboqiang/TjuaeUI/releases/download/');

      const linuxAsset = assets.find((a: { name: string }) => a.name === 'TjuaeUI-1.9.22-linux-x64.deb');
  ...
  it('preserves the release tag in the GitHub download path', async () => {
    ...
    try {
      ...
      expect(asset?.url).toMatch(/^https:\/\/github\.com\/liangboqiang\/TjuaeUI\/releases\/download\/v1\.9\.22\//);
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it('accepts GitHub release URLs for download', async () => {
    ...
    try {
      ...
      const result = await handler({
        ...
        url: 'https://github.com/liangboqiang/TjuaeUI/releases/download/v1.9.22/TjuaeUI-1.9.22-mac-arm64.dmg',
        file_name: 'TjuaeUI-1.9.22-mac-arm64.dmg',
      });
