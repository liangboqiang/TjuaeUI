...
const makeAppDataDir = () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'tjuaeui-installer-last-failure-'));
  tempDirs.push(dir);
  return dir;
};
...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData TjuaeUI directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'TjuaeUI.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'TjuaeUI.exe' }]);
