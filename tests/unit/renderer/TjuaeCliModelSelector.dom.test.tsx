...
import type { TjuaeCliModelSelection } from '@/renderer/pages/conversation/platforms/tjuaecli/useTjuaeCliModelSelection';
...
const makeSelection = (overrides: Partial<TjuaeCliModelSelection> = {}): TjuaeCliModelSelection => ({
  current_model: {
    ...provider,
    use_model: 'gpt-5.2',
...
describe('TjuaeCliModelSelector runtime options', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows the current model and thought level in the header pill', () => {
    ...
    expect(screen.getByTestId('tjuaecli-model-selector')).toHaveTextContent('gpt-5.2 · High');
  });
  ...
  it('renders the model list directly (no submenu) when thought level is unavailable', () => {
    render(<TjuaeCliModelSelector selection={makeSelection()} />);

    expect(screen.getByTestId('tjuaecli-model-selector')).toHaveTextContent('gpt-5.2');
