...
vi.mock('@/common', () => ({
  ipcBridge: {
    acpConversation: {
      ...
      checkAllManagedAgentHealth: { invoke: vi.fn() },
    },
...
describe('LocalAgents', () => {
  ...
  it('runs one batch scan and refreshes all management statuses', async () => {
    const refreshCatalog = vi.fn().mockResolvedValue(undefined);
    useManagedAgents.mockReturnValue({ agents: makeAgents(), revalidate: vi.fn(), refreshCatalog });
    vi.mocked(ipcBridge.acpConversation.checkAllManagedAgentHealth.invoke).mockResolvedValue([
      { ...makeAgents()[0], status: 'online' },
      { ...makeAgents()[1], status: 'offline', last_check_error_code: 'auth_required' },
    ]);

    render(<LocalAgents />);
    fireEvent.click(screen.getByTestId('btn-test-all-agents'));

    await waitFor(() => {
      expect(ipcBridge.acpConversation.checkAllManagedAgentHealth.invoke).toHaveBeenCalledTimes(1);
      expect(refreshCatalog).toHaveBeenCalledTimes(1);
      expect(messageSuccess).toHaveBeenCalledWith('settings.agentManagement.testAllComplete');
    });
  });
