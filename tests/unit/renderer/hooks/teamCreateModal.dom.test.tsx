...
// Mirror TjuaeModal's real prop contract: header/footer may be config objects.
...
function assistants(): Assistant[] {
  return [
    ...
    assistant({
      ...
      name: 'Remote Runner',
      source: 'generated',
      engine_id: 'agent-remote',
      engine: { type: 'remote', ownership: 'custom' },
      team_selectable: true,
    }),
  ];
}

function assistant(overrides: Partial<Assistant> & Pick<Assistant, 'id' | 'name' | 'source' | 'engine_id'>): Assistant {
  return {
    id: overrides.id,
