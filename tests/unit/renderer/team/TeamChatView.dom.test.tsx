...
const tjuaecliChatMock = vi.fn(() => <div data-testid='mock-tjuaecli-chat' />);
...
vi.mock('@/renderer/pages/conversation/platforms/tjuaecli/TjuaeCliChat', () => ({
  __esModule: true,
  default: (props: unknown) => tjuaecliChatMock(props),
}));
...
describe('TeamChatView', () => {
  ...
  it('passes loaded skills and MCP snapshot to TjuaeCLI team chat', async () => {
    usePresetAssistantInfoMock.mockReturnValue({ info: null });
    const mcpStatuses = [{ id: 'office', name: 'office', status: 'loaded' as const }];
    ...
    expect(await screen.findByTestId('mock-tjuaecli-chat')).toBeInTheDocument();
    expect(tjuaecliChatMock.mock.calls[0]?.[0]).toEqual(
      expect.objectContaining({
        loadedSkills: ['excel'],
        loadedMcpServers: ['office'],
        loadedMcpStatuses: mcpStatuses,
