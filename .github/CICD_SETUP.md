...
4. 维护者核对安装、启动、自动更新元数据和哈希后发布 Release。
...
仓库检出、标签推送、工作流重跑、公开依赖下载与 Release 创建统一使用 GitHub 自动提供的 `github.token`，不要求自定义凭据。
...
- 不要在仓库中保存 Personal Access Token 或云端长期凭据。
