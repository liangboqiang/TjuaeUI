...
const isE2ETestMode = process.env.TJUAEUI_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.TJUAEUI_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[TjuaeUI] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[TjuaeUI] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
// Keep fatal main-process failures in the local application log while
// preventing Electron's default error dialog from interrupting shutdown.
process.on('uncaughtException', (error) => {
  console.error('[TjuaeUI] Uncaught main-process exception:', error);
});

process.on('unhandledRejection', (reason) => {
  console.error('[TjuaeUI] Unhandled main-process rejection:', reason);
});
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      method: 'POST',
      headers: {
        'x-tjuaeui-internal': '1',
      },
    }).catch((error) => {
      console.error('[TjuaeUI] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[TjuaeUI] runBackendMigrations completed');
    } catch (error) {
      console.error('[TjuaeUI] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[TjuaeUI] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  const reason = process.env.TJUAEUI_DEBUG_BACKEND_STARTUP_FAILURE as BackendStartupFailureInfo['reason'] | undefined;
  if (!reason) {
    return null;
  }
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[TjuaeUI] Ignoring TJUAEUI_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[TjuaeUI] Ignoring unknown TJUAEUI_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[TjuaeUI] Creating main window...');
  ...
  console.log(`[TjuaeUI] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[TjuaeUI] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[TjuaeUI] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[TjuaeUI] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[TjuaeUI] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[TjuaeUI] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[TjuaeUI] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[TjuaeUI] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[TjuaeUI] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[TjuaeUI] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[TjuaeUI] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[TjuaeUI] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[TjuaeUI] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[TjuaeUI] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[TjuaeUI] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[TjuaeUI] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[TjuaeUI] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[TjuaeUI:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start tjuaecore only after initializeProcess(). initStorage may open
  ...
  try {
    ...
    console.error('[TjuaeUI] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[TjuaeUI] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`TjuaeUI --webui` or packaged `tjuaeui-web` mode that
      ...
      const handle = await startWebHost({
        ...
        // Expose the same TJUAEUI_{CACHE,WORK,LOG}_DIR env the desktop IPC path
...
// Register tjuae:// as the default protocol client
...
// macOS: handle tjuae:// URLs via the open-url event
...
// instance lock. A lock-losing instance must NOT spawn a competing tjuaecore
// backend — doing so races the first instance's tjuaecore over the same data
// directory and previously produced a false "local data repair failed" alarm.
// Gating here (rather than at the top-level second-instance block) keeps it
// after handleAppReady is declared.
if (shouldRegisterBackendStartup(gotTheLock)) {
  void app
    ...
    .catch((error) => {
      ...
      console.error('[TjuaeUI] App initialization failed:', error);
      app.quit();
    });
}
...
app.on('will-quit', () => {
  console.log('[TjuaeUI] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[TjuaeUI] quit (exitCode=${exitCode})`);
});
