...
const AgentRepairPanel: React.FC<AgentRepairPanelProps> = ({ agent, onSaved }) => {
  ...
  const envBlock = (
    <div>
      ...
      {/* What configuring env vars can fix — grounded in how TjuaeUI injects them
  ...
  return (
    <div className='mt-10px flex flex-col gap-12px rounded-10px bg-aou-1 px-12px py-12px'>
      ...
      {!isInternalTjuaeCli && isActionable && showPath ? pathBlock : null}
      {!isInternalTjuaeCli && isActionable ? envBlock : null}
      ...
      {!isInternalTjuaeCli && isActionable ? (
        <Button
          type='primary'
          size='large'
          disabled={isSaving}
