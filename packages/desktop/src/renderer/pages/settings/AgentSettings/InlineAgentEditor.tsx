...
const InlineAgentEditor: React.FC<InlineAgentEditorProps> = ({ agent, onSave, onCancel }) => {
  ...
  const [protocol, setProtocol] = useState<'acp' | 'a2a'>('acp');
  ...
  useEffect(() => {
    ...
    if (agent) {
      ...
      setName(agent.name || '');
      setProtocol(agent.agent_type === 'a2a' ? 'a2a' : 'acp');
      setCommand(agent.command || '');
      setEndpoint(agent.agent_source_info?.endpoint || '');
      setAuthType(
        agent.agent_source_info?.auth_type === 'bearer' || agent.agent_source_info?.auth_type === 'api_key'
          ? agent.agent_source_info.auth_type
          : 'none'
      );
      // 密钥永不返回渲染进程。编辑时留空可保留原凭据，输入新值则替换。
      setAuthToken('');
      setAllowInsecure(agent.agent_source_info?.allow_insecure === true);
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    try {
      if (protocol === 'a2a') {
        const result = await acpConversation.testA2aAgent.invoke({
  ...
  }, [protocol, endpoint, authType, authToken, allowInsecure, name, command, argsString, envVars, runtimeScopeId]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    const draft: CustomAgentDraft = {
      ...
      icon: avatar,
      protocol,
      command: protocol === 'acp' ? command.trim() : '',
      endpoint: protocol === 'a2a' ? endpoint.trim() : undefined,
      auth_type: protocol === 'a2a' && authType !== 'none' ? authType : undefined,
      auth_token: protocol === 'a2a' && authToken.trim() ? authToken.trim() : undefined,
      allow_insecure: protocol === 'a2a' ? allowInsecure : undefined,
      enabled: agent?.enabled !== false,
      args: protocol === 'acp' && parsedArgs.length > 0 ? parsedArgs : undefined,
      env: protocol === 'acp' && envEntries.length > 0 ? envEntries : undefined,
      advanced: protocol === 'acp' && hasAdvanced ? advanced : undefined,
    };
    onSave(draft);
  }, [
    agent,
  ...
  const missingConnectionTarget = protocol === 'acp' ? !command.trim() : !endpoint.trim();
  const isSubmitDisabled = !name.trim() || missingConnectionTarget;
  const isTestDisabled = missingConnectionTarget || testStatus === 'testing';
  ...
  return (
    <div className='flex flex-col gap-16px pt-8px pb-20px'>
      <div>
        <Typography.Text className={fieldLabelClassName}>
          {t('settings.agentProtocol', { defaultValue: '连接协议' })}
        </Typography.Text>
        <Radio.Group
          type='button'
          value={protocol}
          onChange={(value) => {
            setProtocol(value as 'acp' | 'a2a');
            setTestStatus('idle');
            setTestErrorDetail('');
          }}
          className='w-full [&_.arco-radio-button]:flex-1 [&_.arco-radio-button]:text-center'
        >
          <Radio value='acp'>{t('settings.agentProtocolAcp', { defaultValue: 'ACP · 本地进程' })}</Radio>
          <Radio value='a2a'>{t('settings.agentProtocolA2a', { defaultValue: 'A2A · 远程服务' })}</Radio>
        </Radio.Group>
        <Typography.Text type='secondary' className={fieldHelpClassName}>
          {protocol === 'acp'
            ? t('settings.agentProtocolAcpHelp', { defaultValue: '启动本地命令，并通过 ACP 标准输入输出协议通信。' })
            : t('settings.agentProtocolA2aHelp', {
                defaultValue: '连接远程服务并读取 A2A Agent Card，不会启动本地程序。',
              })}
        </Typography.Text>
      </div>
      ...
      {protocol === 'acp' ? (
        ...
        <>
          <div>
            <Typography.Text className={fieldLabelClassName}>
              {t('settings.a2aEndpoint', { defaultValue: 'Agent Card 地址' })}
            </Typography.Text>
            <Input size='large' value={endpoint} onChange={setEndpoint} placeholder='https://agent.example.com' />
            <Typography.Text type='secondary' className={fieldHelpClassName}>
              {t('settings.a2aEndpointHelp', {
                defaultValue: '可填写 Agent Card 完整地址，也可填写服务根地址由 TjuaeUI 自动发现。',
              })}
            </Typography.Text>
          </div>
          <div>
            <Typography.Text className={fieldLabelClassName}>
              {t('settings.a2aAuthType', { defaultValue: '认证方式' })}
            </Typography.Text>
            <Select value={authType} onChange={(value) => setAuthType(value as typeof authType)} className='w-full'>
              <Select.Option value='none'>{t('settings.a2aAuthNone', { defaultValue: '无认证' })}</Select.Option>
          ...
          {authType !== 'none' && (
            <div>
              <Typography.Text className={fieldLabelClassName}>
                {t('settings.a2aAuthToken', { defaultValue: '认证凭据' })}
              </Typography.Text>
              <Input.Password
                size='large'
                value={authToken}
                ...
                placeholder={
                  agent
                    ? t('settings.a2aAuthTokenKeep', { defaultValue: '留空以保留现有凭据' })
                    : t('settings.a2aAuthTokenPlaceholder', { defaultValue: '输入 Token 或 API Key' })
                }
              />
            </div>
          )}
          <div className='flex items-center justify-between gap-16px rounded-10px border border-solid border-[var(--color-border-2)] bg-[var(--color-fill-1)] px-14px py-12px'>
            <div>
              <Typography.Text className='block text-13px font-medium text-t-primary'>
                {t('settings.a2aAllowInsecure', { defaultValue: '允许 HTTP 连接' })}
              </Typography.Text>
              <Typography.Text type='secondary' className={fieldHelpClassName}>
                {t('settings.a2aAllowInsecureHelp', { defaultValue: '仅用于可信内网；公网服务应始终使用 HTTPS。' })}
              </Typography.Text>
            </div>
            <Switch checked={allowInsecure} onChange={setAllowInsecure} />
          </div>
        </>
      )}
      ...
      <div>
        ...
        {testStatus === 'success' && (
          <Alert
            ...
            content={
              protocol === 'a2a'
                ? t('settings.testConnectionSuccessA2a', { defaultValue: '连接成功，已读取 A2A Agent Card。' })
          ...
          <Alert
            ...
            content={
              <div className='flex flex-col gap-4px'>
                <span>
                  {protocol === 'a2a'
                    ? t('settings.testConnectionFailA2a', { defaultValue: '无法连接或解析 A2A Agent Card' })
                    : t('settings.testConnectionFailCli')}
                </span>
