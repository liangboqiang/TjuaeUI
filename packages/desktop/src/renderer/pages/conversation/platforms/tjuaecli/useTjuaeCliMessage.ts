...
export const useTjuaeCliMessage = (
  ...
  useEffect(() => {
    return ipcBridge.conversation.responseStream.on((message) => {
      ...
      switch (message.type) {
        ...
        case 'finish':
          {
            logStreamTerminalObserved(conversation_id, message.turn_id, 'tjuaecli', message.type);
            // tjuaecli stream_end carries usage in data field
        ...
        case 'acp_permission':
          ...
          // Backend tjuaecli emits wire type 'acp_permission' but the payload is
  ...
  useEffect(() => {
    ...
    void getConversationOrNull(conversation_id).then((res) => {
      ...
      if (res.type === 'tjuaecli' && res.extra?.last_token_usage) {
