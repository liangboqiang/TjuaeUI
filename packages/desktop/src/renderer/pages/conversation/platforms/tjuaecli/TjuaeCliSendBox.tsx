...
const useTjuaeCliSendBoxDraft = getSendBoxDraftHook('tjuaecli', {
  _type: 'tjuaecli',
  atPath: [],
  content: '',
  uploadFile: [],
...
const useSendBoxDraft = (conversation_id: string) => {
  const { data, mutate } = useTjuaeCliSendBoxDraft(conversation_id);
...
const TjuaeCliSendBox: React.FC<{
  conversation_id: string;
  modelSelection: TjuaeCliModelSelection;
  session_mode?: string;
  agent_name?: string;
  ...
  const { thought, running, setActiveMsgId, setWaitingResponse, resetState } = useTjuaeCliMessage(conversation_id, {
    ...
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      try {
        ...
        if (teamSendMessage) {
          ...
          if (files.length > 0) {
            emitter.emit('tjuaecli.workspace.refresh');
          }
          return;
        }
  ...
  useEffect(() => {
    ...
    const storageKey = `tjuaecli_initial_message_${conversation_id}`;
    const processedKey = `tjuaecli_initial_processed_${conversation_id}`;
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[TjuaeCliSendBox] Failed to send initial message:', error);
  ...
  const onSendHandler = async (message: string) => {
    ...
    clearFiles();
    emitter.emit('tjuaecli.selected.file.clear');
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[TjuaeCliSendBox] Failed to switch mode via sheet:', error);
        Message.error(t(configErrorMessageKey(error)));
      }
    },
  ...
  useAddEventListener('tjuaecli.selected.file', setAtPath);
  useAddEventListener('tjuaecli.selected.file.append', (selectedItems: Array<string | FileOrFolderItem>) => {
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[TjuaeCliSendBox] stop request failed', error);
    ...
    <div className={`${sendBoxWidthClass} flex flex-col mt-auto mb-16px`}>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(items) => {
          emitter.emit('tjuaecli.selected.file', items);
          setAtPath(items);
        }}
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`tjuaecli-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`tjuaecli-folder-tag-${folderIndex}`}
                        color='blue'
                        closable
                        onClose={() => {
                          const newAtPath = atPath.filter((v) => (typeof v === 'string' ? true : v.path !== item.path));
                          emitter.emit('tjuaecli.selected.file', newAtPath);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
...
export default TjuaeCliSendBox;
