...
const messageText = (message: TMessage): string => {
  ...
  if (message.type === 'tool_group') {
    return message.content.map((tool) => tool.name || tool.description).filter(Boolean).join('、');
  }
  ...
  if (message.type === 'permission' || message.type === 'acp_permission') {
    return message.type === 'permission' ? message.content.description : String(message.content.title ?? '');
  }
...
const traceColor = (message: TMessage): string => {
  ...
  if (message.type === 'tool_call' || message.type === 'tool_group' || message.type === 'acp_tool_call') return 'arcoblue';
  ...
  return (
    <>
      ...
      >
        ...
        {isLoading && !data ? (
          <div className='flex justify-center py-48px'><Spin /></div>
        ) : error ? (
            ...
            {events.map((event) => {
              ...
              return (
                <Timeline.Item key={event.id} dotColor={traceColor(event)}>
                  ...
                  {preview && <Typography.Paragraph ellipsis={{ rows: 3, expandable: true }}>{preview}</Typography.Paragraph>}
                </Timeline.Item>
              );
            })}
          </Timeline>
