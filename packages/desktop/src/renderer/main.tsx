...
import { MANAGED_AGENTS_SWR_KEY, type ManagedAgent } from './utils/model/agentTypes';
...
const Main = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void ipcBridge.acpConversation.checkAllManagedAgentHealth
      .invoke()
      .then(async (agents) => {
        if (cancelled) return;
        await Promise.all([
      ...
      .catch((error) => console.warn('Agent 启动扫描与目录预加载失败：', error));
    return () => {
      cancelled = true;
    };
  }, [configReady, ready]);
