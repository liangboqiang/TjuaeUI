...
const setDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'tjuaecli':
      if (draft) {
        store.tjuaecli.set(conversation_id, draft as Extract<Draft, { _type: 'tjuaecli' }>);
      } else {
        store.tjuaecli.delete(conversation_id);
      }
      break;
    default:
      break;
...
const getDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'tjuaecli':
      return store.tjuaecli.get(conversation_id) as Extract<Draft, { _type: K }>;
    default:
      return undefined;
  }
};
