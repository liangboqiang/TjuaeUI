...
declare global {
  interface Window {
    __TjuaeSafeResizeObserver__?: boolean;
    __TjuaeResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__TjuaeSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__TjuaeSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__TjuaeResizeObserverPatched__) {
    ...
    window.__TjuaeResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__TjuaeResizeObserverPatched__) {
    ...
    console.__TjuaeResizeObserverPatched__ = true;
  }
};
