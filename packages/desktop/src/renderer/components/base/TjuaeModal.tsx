...
/** TjuaeModal 组件 Props */
export interface TjuaeModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_HEADER_CLASS = 'tjuaeui-modal-std-header flex items-start justify-between gap-16px px-24px pt-20px pb-16px';
...
const STD_BODY_LAYOUT_CLASS = 'tjuaeui-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <TjuaeModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </TjuaeModal>
 *
 * // 预设尺寸 / Preset size
 * <TjuaeModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </TjuaeModal>
 *
...
const TjuaeModal: React.FC<TjuaeModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'tjuaeui-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('tjuaeui-modal', isStandard && 'tjuaeui-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('tjuaeui-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'tjuaeui-modal-body-content',
...
TjuaeModal.displayName = 'TjuaeModal';

export default TjuaeModal;
