 ...
 */
export interface TjuaeCollapseProps {
 ...
 */
export interface TjuaeCollapseItemProps {
 ...
 */
const TjuaeCollapseItem: React.FC<TjuaeCollapseItemProps> = ({ children }) => <>{children}</>;
TjuaeCollapseItem.displayName = 'TjuaeCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <TjuaeCollapse defaultActiveKey={['1']}>
 *   <TjuaeCollapse.Item name="1" header="面板1">
 *     内容1
 *   </TjuaeCollapse.Item>
 *   <TjuaeCollapse.Item name="2" header="面板2">
 *     内容2
 *   </TjuaeCollapse.Item>
 * </TjuaeCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <TjuaeCollapse accordion defaultActiveKey="1">
 *   <TjuaeCollapse.Item name="1" header="面板1">内容1</TjuaeCollapse.Item>
 *   <TjuaeCollapse.Item name="2" header="面板2">内容2</TjuaeCollapse.Item>
 * </TjuaeCollapse>
 *
 * // 自定义图标 / Custom icon
 * <TjuaeCollapse
 ...
 * ```
 */
const TjuaeCollapseComponent: React.FC<TjuaeCollapseProps> & { Item: typeof TjuaeCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<TjuaeCollapseItemProps> => {
      return React.isValidElement(child) && child.type === TjuaeCollapseItem;
    });
  }, [children]);
...
TjuaeCollapseComponent.Item = TjuaeCollapseItem;

export default TjuaeCollapseComponent;
