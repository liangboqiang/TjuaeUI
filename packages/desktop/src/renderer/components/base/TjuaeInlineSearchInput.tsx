...
import styles from './TjuaeInlineSearchInput.module.css';

/**
 * TjuaeInlineSearchInput —— 下拉列表专用的轻量搜索框
 *
 * 样式基准取自首页 Project 选择下拉曾用的搜索框：浅灰填充、无边框、8px 圆角、
 * 放大镜图标 + 透明输入框，柔和紧凑，适合“点击展开的下拉列表”顶部内嵌使用。
 * 与 TjuaeSearchInput（34px 描边、focus 主色环，适合常驻搜索栏）区分：
 * 常驻列表用 TjuaeSearchInput，点击触发展开的下拉列表用本组件。
 *
 * API 与 TjuaeSearchInput 对齐（value/onChange/placeholder），只负责外观与录入，
 * 过滤/防抖等逻辑仍由调用方持有。
 */
export type TjuaeInlineSearchInputProps = {
  /** 当前输入值（受控） */
  value: string;
  /** 值变化回调，返回最新字符串 */
...
const TjuaeInlineSearchInput = forwardRef<HTMLInputElement, TjuaeInlineSearchInputProps>((props, ref) => {
...
TjuaeInlineSearchInput.displayName = 'TjuaeInlineSearchInput';

export default TjuaeInlineSearchInput;
