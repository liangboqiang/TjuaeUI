...
// BEFORE any getPath() call so the whole data tree (config, tjuaecore DB, logs)
// lives in a disposable directory. This keeps tests off the developer's real
// database — critical because TjuaeCore refuses to boot when a shared DB fails
// migration. Guarded by TJUAEUI_E2E_TEST so it never affects dev/production.
// 仅 E2E：把 userData 指向一次性沙箱目录，避免测试读写真实数据库。
const e2eUserDataDir = process.env.TJUAEUI_E2E_TEST === '1' ? process.env.TJUAEUI_E2E_USER_DATA_DIR : undefined;
...
// Override via TJUAEUI_CDP_PORT env variable. Set to "0" to disable.
//
...
// Registry file: ~/.tjuaeui-cdp-registry.json
// ---------------------------------------------------------------------------

export const DEFAULT_CDP_PORT = 9230;
export const CDP_PORT_RANGE_START = 9230;
export const CDP_PORT_RANGE_END = 9250;
const CDP_REGISTRY_FILE = path.join(os.homedir(), '.tjuaeui-cdp-registry.json');
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another TjuaeUI instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active TjuaeUI instances, trying ${preferredPort}`
  );
  return preferredPort;
}
...
function resolveCdpPortFromEnv(): number | null | undefined {
  const envVal = process.env.TJUAEUI_CDP_PORT;
