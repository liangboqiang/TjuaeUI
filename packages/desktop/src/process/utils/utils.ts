...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'tjuaeui');
};
 ...
 * Release builds use ~/.tjuaeui; dev builds use ~/.tjuaeui-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.tjuaeui，Dev 模式使用 ~/.tjuaeui-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'tjuaeui');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.tjuaeui'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.tjuaeui-config; dev builds use ~/.tjuaeui-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.tjuaeui-config，Dev 模式使用 ~/.tjuaeui-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.tjuaeui-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[TjuaeUI] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[TjuaeUI] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[TjuaeUI] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[TjuaeUI] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
