...
async function syncBuiltinMcpConfig(configFile: ConfigFile): Promise<void> {
  ...
  console.info(
    '[TjuaeUI] Synced builtin MCP config to backend settings (%d builtin servers)',
...
export async function runBackendMigrations(configFile: ConfigFile): Promise<void> {
  ...
  try {
    ...
    console.info(`[TjuaeUI] Backend migration step completed: syncBuiltinMcpConfig (${Date.now() - syncStart}ms)`);
  } catch (error) {
    console.error(`[TjuaeUI] Backend migration step failed: syncBuiltinMcpConfig (${Date.now() - syncStart}ms)`, error);
  }
}
