 ...
 * /login and /logout to tjuaecore. All auth goes to backend's tjuaeui-auth crate;
 * /login and /logout are tjuaeui-auth's top-level paths, the rest live under
...
export async function startStaticServer(opts: StaticServerOptions): Promise<StaticServerHandle> {
  ...
  const http_server: Server = http.createServer(async (req, res) => {
    try {
      ...
      // /login and /logout are tjuaeui-auth's top-level auth endpoints: proxy them too
